...
namespace coen79_lab8
{
    ...
    public:
        ...
        deque_iterator& operator ++( ) // Prefix ++
        {
            ...
            {
                current_block_pointer++;   //move the current block ptr to the start of the next data block
                current_boundary = *current_block_pointer + block_size - 1; //update current boundary to the next data block's boundary
        ...
        deque_iterator operator ++(int) // Postfix ++
        {
            ...
            {
                // STUDENT WORK...
                current_block_pointer++;    //move the current block ptr 
                current_boundary = *current_block_pointer + block_size - 1;   //update current boundary to the next data block's boundary
                cursor = *current_block_pointer;   
            }
            // Move forward
