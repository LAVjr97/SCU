...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        value_type **block_pointers = new value_type *[bp_array_size];
    ...
    void deque<Item>::clear () {
        ...
        // Clear the data blocks
        // STUDENT WORK...
        for(i = 0; i < bp_array_size; i++) //iterates through the bp
            if(block_pointers[i] != NULL){ 
                delete [] block_pointers[i]; //delete current item
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            // STUDENT WORK...
            *first_bp = new value_type[block_size]; //create new block in array
            front_ptr = *first_bp;  //set front_ptr to the start
            *front_ptr = entry;
            back_ptr = *last_bp;
        }
        ...
        else if (front_ptr != *first_bp)    //data block has space for a new entry
        {
            // STUDENT WORK...
            front_ptr--;
            *front_ptr = entry;
        ...
        else if ((*first_bp == front_ptr) && (first_bp != block_pointers)) //data block is full but block isn't full
        {
            // STUDENT WORK...
            first_bp--; //move first_bp to a new index
            *first_bp = new value_type[block_size]; //create new block in array
            front_ptr = *first_bp + block_size - 1; //set front_ptr to the last item in the new data block
            *front_ptr = entry;
        ...
        else if ((*first_bp == front_ptr) && (first_bp == block_pointers)) //at the start of the array (not enough space)
        {
            // STUDENT WORK...
            reserve();  //increases array size
            first_bp--;
