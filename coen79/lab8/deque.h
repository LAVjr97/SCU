...
namespace coen79_lab8
{
    ...
    void deque<Item>::clear () {
        ...
        // Clear the data blocks
        // STUDENT WORK...
        for(i = 0; i < bp_array_size; i++) //iterates through the block pointer array
            if(block_pointers[i] != NULL){ 
                delete [] block_pointers[i]; //delete the data block array
    ...
    void deque<Item>::reserve()
    {
        ...
        copy(first_bp, last_bp + 1, new_block_pointers + 10 + offsett_first_bp);
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            // STUDENT WORK..
            //set first_bp ptr to new block, then front_ptr to first_bp pointer
            //set back_ptr to last_bp ptr, then front_ptr ptr to entry.
        ...
        {
            // STUDENT WORK...
            //decrement front_ptr, set its pointer to entry
            front_ptr--;
            *front_ptr = entry;
        }
        ...
        {
            // STUDENT WORK...
            //decrement first_bp, then set pointer of it to new block
            //allocate space by setting front_ptr to first_bp pointer + size - 1
            //set pointer of front_ptr to entry
            first_bp--;
            *first_bp = new value_type[BLOCK_SIZE];
        ...
        {
            // STUDENT WORK...
            //calls reserve function then decrements first_bp
            //sets pointer to new block, front_ptr to first_bp ptr + size - 1 to allocate space
            //set front ptr to entry
            reserve();
            first_bp--;
            *first_bp = new value_type[BLOCK_SIZE];
            front_ptr = *first_bp + BLOCK_SIZE - 1;
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            // STUDENT WORK...
            //sets last_bp pointer to new block, front_ptr = first_bp ptr
            //then set back_ptr = to last_bp ptr, then back_ptr pointer = entry.
        ...
        {
            // STUDENT WORK...
            //increment back_ptr by 1, sets the pointer = to entry
            back_ptr++;
            *back_ptr = entry;
        }
        ...
        {
            // STUDENT WORK...
            //increments last_bp by 1, sets pointer to new block, then back_ptr = to pointer of last_bp. 
            //back_ptr pointer now = entry
            last_bp++;
            *last_bp = new value_type[BLOCK_SIZE];
            back_ptr = *last_bp;
            *back_ptr = entry;
        ...
        {
            // STUDENT WORK...
            //reserve data by calling reserve function, increment last_bp. set last_bp ptr to new block, make back_ptr = last_bp ptr
            //set back ptr = to entry
            reserve();
            last_bp++;
            *last_bp = new value_type[BLOCK_SIZE];
            back_ptr = *last_bp;
    ...
    void deque<Item>::pop_front()
    {
        ...
        {
            // STUDENT WORK...
            //clear data by calling clear function
            clear();
        }
        ...
        {
            // STUDENT WORK...
            //delete array pointer, set first_bp to NULL then increment. set front_ptr to pointer of first_bp
            delete [] *first_bp;
            *first_bp = NULL;
            first_bp++;
            front_ptr = *first_bp;
    ...
    void deque<Item>::pop_back()
    {
        ...
        {
            // STUDENT WORK...
            //delete array pointer, set pointer to NULL and decrement it. Set back ptr to last_bp ptr + block_size - 1
            delete [] *last_bp;
            *last_bp = NULL;
            last_bp--;
